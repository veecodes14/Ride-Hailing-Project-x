...
export const changePassword = async (req: AuthRequest, res: Response): Promise <void> => {
    try {
        ...
        const isPasswordValid = await bcrypt.compare(oldPaassword, user.password)
        if (!isPasswordValid) {
            res.status(400).json({
                success: false,
...
export const deleteAccount = async(req: AuthRequest, res: Response): Promise<void> => {
    try {
        ...
        res.status(500).json({ success: false, error: "Internal Server zerror"})
        return
    }
}